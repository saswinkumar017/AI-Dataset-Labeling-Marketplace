...
/**
 * Proves verified exports carry the full dataset-item payload: tabular rows
 * export their column map plus the ordered header, and image items export
 * their stored URL — alongside the legacy snapshots older consumers read.
 */
@SpringBootTest
@AutoConfigureMockMvc
class ExportWithItemsIntegrationTest {
    ...
    void shouldExportFullRowDataAndColumns() throws Exception {
        ...
        // The verified row carries the full tabular payload, not just text.
        mockMvc.perform(get("/api/projects/" + projectId + "/export")
                        .header("Authorization", "Bearer " + owner))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(1))
                .andExpect(jsonPath("$[0].label").value("Positive"))
                .andExpect(jsonPath("$[0].rowData.review").value("Loved it"))
                .andExpect(jsonPath("$[0].rowData.rating").value("5"))
                .andExpect(jsonPath("$[0].columns[0]").value("review"))
                .andExpect(jsonPath("$[0].columns[1]").value("rating"))
                .andExpect(jsonPath("$[0].content").value("review: Loved it | rating: 5"));
    }
}
